...
object IndentedParagraphsMerger {
  ...
  class ApproximateFloat(val self: Float) extends FractionalProxy[Float] {
    ...
    implicit protected val num: Fractional[Float] = Numeric.FloatIsFractional
    implicit protected val integralNum = Numeric.FloatAsIfIntegral
    implicit protected val ord: Ordering[Float] = ???
