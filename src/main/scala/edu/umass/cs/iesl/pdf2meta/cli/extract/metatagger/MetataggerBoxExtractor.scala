...
class MetataggerBoxExtractor extends MetataggerExtractor with Logging with Function1[Workspace,ClassifiedRectangles /*(DocNode, ClassifiedRectangles)*/] {
  ...
  val mapAcceptedLabels:Map[String, String] = Map("CONTENT -> HEADERS -> TITLE" -> "HEADERS -> TITLE",
   ...
   // "CONTENT -> HEADERS -> NOTE" -> "HEADERS -> NOTE",
    ...
    "CONTENT -> BODY -> SECTION-MARKER" -> "BODY -> SECTION-MARKER",
    "CONTENT -> BODY -> TABLE-MARKER" -> "BODY -> TABLE-MARKER",
    "CONTENT -> BODY -> FIGURE-MARKER" -> "BODY -> FIGURE-MARKER",
    "CONTENT -> BODY -> PARAGRAPH-MARKER" -> "BODY -> PARAGRAPH-MARKER"
    )
...
//                                  "CONTENT -> HEADERS -> NOTE -> INSTITUTION",
 //                                 "CONTENT -> HEADERS -> NOTE -> ADDRESS",
  ...
  val includeCoordinates:List[String] = List("REFERENCE"/*, "TEXT"*/)

  val includePage:List[String] = List("SECTION-MARKER", "TABLE-MARKER", "FIGURE-MARKER")

  val idAttributes:Map[String, List[String]] = Map("PARAGRAPH-MARKER" -> List("id"))
  ...
  def apply(v1: Workspace) = {
    ...
    val docNodes:Seq[ClassifiedRectangle] =
      processXMLRecursiveV6(documentMT \\ "content", "", "",
     new Rectangle {
    ...
    val classifiedRectangles:ClassifiedRectangles = new ClassifiedRectangles(docNodes/*.reverse*/)
  ...
  {
    val id:String = (mapData(0) \ ("@id")).text

    val label:String = mapData(0).label;

    val retText:String = mapData.map(x=>
      //groups by ids
      if(!id.equals("") && (x \ "@id").text.equals(id))
        {
       x.text}
      else if(id.equals(""))
      {
...
//    //todo: here a list can be implemented that includes everything with "REFERENCE", "BODY" ,...., and so on :)
//
...
//  def processXMLRecursiveV5(node:Seq[Node], parentName:String, parentId:String, pageDimensions:Rectangle, justContent:Boolean):Seq[ClassifiedRectangle] =
//  {
//
//    //    println("parent data: " + parentName + " " + parentId)
//
...
//    //todo: here a list can be implemented that includes everything with "REFERENCE", "BODY" ,...., and so on :)
//
//    def returnParentId(lblName:String, rect:RectangleOnPage, parentId:String)={if(includeCoordinates.contains(lblName.toUpperCase())){lblName.toUpperCase() + "_" + rect.top.toInt + "_" +
//      rect.left.toInt + "_" + rect.bottom.toInt + "_" + rect.right.toInt + "_" + rect.page.pagenum +  "_"}else{parentId}}
//
//    val res = for(currentNode <- node.groupBy(_.label))
//    yield
...
//          Math.abs((currentNode._2(0) \ "@lly").text.toFloat - (currentNode._2(0) \ "@ury").text.toFloat)> 400 /*400*/)
//        {
//          val recRes:Seq[ClassifiedRectangle] = processXMLRecursiveV5(currentNode._2(0).child, parentName + currentNode._2(0).label + " -> ",parentId,pageDimensions,false)
//          val recSiblings:Seq[ClassifiedRectangle] = {if(currentNode._2.size>1){processXMLRecursiveV5(currentNode._2.tail, parentName,parentId,pageDimensions,false)}
//          else
//          {(seqClassifiedRectangle)}}
//
//          ((seqClassifiedRectangle ++ recSiblings) ++ recRes)
...
//            //def getHierarchicalStructure()
//
//            val recRes = processXMLRecursiveV5(currentNode._2(0).child, parentName + currentNode._2(0).label + " -> ",returnParentId(currentNode._2(0).label, currNode.rectangle.get,parentId),pageDimensions,false)
//
//            val recSiblings:Seq[ClassifiedRectangle] = {if(currentNode._2.size>1){processXMLRecursiveV5(currentNode._2.tail, parentName,parentId,pageDimensions,
//              if(siblingHierarchyApplicable()){true}else{false})}
//            else
//            {(seqClassifiedRectangle)}}
//
...
//          else
//          {
//            val recRes:(Seq[ClassifiedRectangle]) = processXMLRecursiveV5(currentNode._2(0).child, parentName + currentNode._2(0).label + " -> ",returnParentId(currentNode._2(0).label, currNode.rectangle.get,parentId),pageDimensions,false)
//            val recSiblings:( Seq[ClassifiedRectangle]) = {if(currentNode._2.size>1){processXMLRecursiveV5(currentNode._2.tail, parentName,parentId,pageDimensions,false)}
//            else
//            {(seqClassifiedRectangle)}}
//
...
//        }
//      case _ =>
//        val recRes = processXMLRecursiveV5(currentNode._2(0).child, parentName + currentNode._2(0).label + " -> ", parentId
//          ,pageDimensions,false)
//        (seqClassifiedRectangle ++ recRes)
//    }
//    }
...
//  }
  ...
  def processXMLRecursiveV6(node:Seq[Node], parentName:String, parentId:String, pageDimensions:Rectangle, justContent:Boolean):Seq[ClassifiedRectangle] =
  {
    ...
    def returnParentId(lblName:String, rect:RectangleOnPage, parentId:String, cNode:Node):String={
      if(includeCoordinates.contains(lblName.toUpperCase()))
      {
        lblName.toUpperCase() + "_" + rect.top.toInt + "_" +
          rect.left.toInt + "_" + rect.bottom.toInt + "_" + rect.right.toInt + "_" + rect.page.pagenum +  "_"
      }
      else if(includePage.contains(lblName.toUpperCase()))
      {
        lblName.toUpperCase() + "_" + rect.page
      }
      else
      {
        //some nodes require certain attributes to be set, for example the <paragraph-marker> node has id attribute to identify each of the paragraphs

        if(idAttributes.contains(lblName.toUpperCase()))
        {
          val attrs:List[String] = idAttributes.get(lblName.toUpperCase()).get
          lblName.toUpperCase() + attrs.map(x=>
              (cNode \ ("@" + x)).text
          ).mkString("_")
        }
        else
...
//        println("change")
        ...
        def getAttrValue(cNode:Node, attr:String):String = {
          (cNode \ ("@" + attr)).text
        }
        ...
        if((parentName + currentNode._1).toUpperCase().contains("REFERENCE") &&
          Math.abs((currentNode._2(0) \ "@lly").text.toFloat - (currentNode._2(0) \ "@ury").text.toFloat)> 400 /*400*/)
        {
          val recRes:Seq[ClassifiedRectangle] = processXMLRecursiveV6(currentNode._2(0).child, parentName + currentNode._2(0).label + " -> ",parentId,pageDimensions,false)
          val recSiblings:Seq[ClassifiedRectangle] = {if(currentNode._2.size>1){processXMLRecursiveV6(currentNode._2.tail, parentName,parentId,pageDimensions,false)}
          else
          {(seqClassifiedRectangle)}}
        ...
        else
        {
          ...
          val currNode: DocNode = new DelimitingBox(
            returnParentId(currentNode._2(0).label, rectOnPage,"",currentNode._2(0)) + parentId + parentName + currentNode._2(0).label
            , rectOnPage)
          val weightedFeatureSet:WeightedSet[Feature] = new WeightedSet[Feature]{
            val asMap = Map[Feature, Double]()
          }
          ...
          if(mapAcceptedLabels.keys.exists(x => x==(parentName + currentNode._2(0).label).toUpperCase()))
          {
            def getContent(currntNode:scala.xml.Node, currNode:DocNode, completePath:String, includePref:Boolean):String =
            {
              val pref = if(includePref){
                if(currNode.id.toUpperCase().contains("PARAGRAPH-MARKER"))
                {
                  val reg = new scala.util.matching.Regex(""".*PARAGRAPH-MARKER([\d]+).*""", "p_id")
                  val m4 = reg.findAllIn(currNode.id.toUpperCase())
                  if(!m4.isEmpty) {
                     " #" + (m4 group "p_id") + ": "
                  }
                  else
              ...
              if(mergeIsApplicable())
              {
                ...
                (currNode.id.toUpperCase().contains("PARAGRAPH-MARKER"))*/)
              {
                pref + currntNode.text.toString()
              }
              else if(recursiveExtraction.exists(x => x == completePath.toUpperCase()))
            ...
            val recRes = processXMLRecursiveV6(currentNode._2(0).child, parentName + currentNode._2(0).label + " -> ",returnParentId(currentNode._2(0).label, currNode.rectangle.get,parentId,currentNode._2(0)),pageDimensions,false)

            val recSiblings:Seq[ClassifiedRectangle] = {if(currentNode._2.size>1){processXMLRecursiveV6(currentNode._2.tail, parentName,parentId,pageDimensions,
              if(siblingHierarchyApplicable()){true}else{false})}
            else
            {(seqClassifiedRectangle)}}
            ...
            (((seqClassifiedRectangle) ++ {if(mergeIsApplicable()){
              recSiblings
            }else{if(!siblingHierarchyApplicable() || justContent)
              {
                recSiblings
              }
              else
            ...
            }) ++ recRes) :+ currClassifiedRectangle
          ...
          {
            val recRes:(Seq[ClassifiedRectangle]) = processXMLRecursiveV6(currentNode._2(0).child, parentName + currentNode._2(0).label + " -> ",returnParentId(currentNode._2(0).label, currNode.rectangle.get,parentId,currentNode._2(0)),pageDimensions,false)
            val recSiblings:( Seq[ClassifiedRectangle]) = {if(currentNode._2.size>1){processXMLRecursiveV6(currentNode._2.tail, parentName,parentId,pageDimensions,false)}
            else
            {(seqClassifiedRectangle)}}
      ...
      case _ =>
        val recRes = processXMLRecursiveV6(currentNode._2(0).child, parentName + currentNode._2(0).label + " -> ", parentId
          ,pageDimensions,false)
        (seqClassifiedRectangle ++ recRes)
    }
    }
